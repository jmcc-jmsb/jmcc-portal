// ABOUTME: Browser Supabase client for the SPA — publishable key only, so every read is RLS-checked.
...
/* This client is the only one client code may ever hold. It carries the
   publishable key, so the database answers it exactly as far as RLS allows and no
   further — a delegate querying case_materials before release gets zero rows
   here, not a filtered UI. The secret-key client lives in src/lib/server/ and
   cannot be imported from here; astro:env makes that a build error. */
...
export function getSupabase(): SupabaseClient {
  ...
  client = createBrowserClient(PUBLIC_SUPABASE_URL, PUBLIC_SUPABASE_PUBLISHABLE_KEY);
  return client;
}
